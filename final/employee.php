...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    {
        $new_count= $r['employee'];
        $id1=$r['id'];
        
    }
    $new_count++;
    $query1="update counts set employee=$new_count where id=$id1";
...
if($employee>0){
    $stmt=$conn->prepare("update times set employee=employee+? where id=$i");
    $stmt->bind_param("s",$employee_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set employee=(?) where id=$i");
    $stmt->bind_param("s",$employee_time);
    $stmt->execute();
