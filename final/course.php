...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    $new_count++;
    $query1="update counts set course=$new_count where id=$id1";
...
if($course>0){
    $stmt=$conn->prepare("update times set course=course+? where id=$i");
    $stmt->bind_param("s",$course_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set course=(?) where id=$i");
    $stmt->bind_param("s",$course_time);
    $stmt->execute();
