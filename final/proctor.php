...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    $new_count++;
    $query1="update counts set proctor=$new_count where id=$id1";
...
if($proctor>0){
    $stmt=$conn->prepare("update times set proctor=proctor+? where id=$i");
    $stmt->bind_param("s",$proctor_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set proctor=(?) where id=$i");
    $stmt->bind_param("s",$proctor_time);
    $stmt->execute();
