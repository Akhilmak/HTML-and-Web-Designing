...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    $new_count++;
    $query1="update counts set report=$new_count where id=$id1";
...
if($report>0){
    $stmt=$conn->prepare("update times set report=report+? where id=$i");
    ...
    $stmt=$conn->prepare("update times set report=(?) where id=$i");
    $stmt->bind_param("s",$report_time);
    $stmt->execute();
