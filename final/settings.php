...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    {
        $new_count= $r['settings'];
        $id1=$r['id'];
        
    }
    $new_count++;
    $query1="update counts set settings=$new_count where id=$id1";
...
if($settings>0){
    $stmt=$conn->prepare("update times set settings=settings+? where id=$i");
    $stmt->bind_param("s",$settings_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set settings=(?) where id=$i");
    $stmt->bind_param("s",$settings_time);
    $stmt->execute();
