...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    {
        $new_count= $r['greivance'];
        $id1=$r['id'];
        
    }
    $new_count++;
    $query1="update counts set greivance=$new_count where id=$id1";
...
$greivance;
    $greivance_time=$_POST['greivance'];
...
if($greivance>0){
    $stmt=$conn->prepare("update times set greivance=greivance+? where id=$i");
    $stmt->bind_param("s",$greivance_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set greivance=(?) where id=$i");
    $stmt->bind_param("s",$greivance_time);
    $stmt->execute();
