...
else if(mysqli_num_rows($query_run)>0)
{
    ...
    $new_count++;
    $query1="update counts set student=$new_count where id=$id1";
...
if($student>0){
    $stmt=$conn->prepare("update times set student=student+? where id=$i");
    $stmt->bind_param("s",$student_time);
    $stmt->execute();
    ...
    $stmt=$conn->prepare("update times set student=(?) where id=$i");
    $stmt->bind_param("s",$student_time);
    $stmt->execute();
